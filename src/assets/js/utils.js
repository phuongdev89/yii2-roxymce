...
RoxyUtils.GetFileIcon    = function(path) {
	var ret = roxyMceAsset + '/images/filetypes/unknown.png';
	if(fileTypeIcons[RoxyUtils.GetFileExt(path).toLowerCase()]) {
		ret = roxyMceAsset + '/images/filetypes/' + fileTypeIcons[RoxyUtils.GetFileExt(path).toLowerCase()];
	}
...
RoxyUtils.LoadLang    = function() {
	...
	if(RoxyFilemanConf.LANG && RoxyFilemanConf.LANG.toLowerCase() == 'auto') {
		if(RoxyUtils.GetUrlParam('langCode')) {
			langUrl = roxyMceAsset + '/lang/' + RoxyUtils.GetUrlParam('langCode').substr(0, 2).toLowerCase() + '.json';
		}
		else {
			var language = window.navigator.userLanguage || window.navigator.language;
			langUrl      = roxyMceAsset + '/lang/' + language.substr(0, 2) + '.json';
		}
		...
		if(RoxyFilemanConf.LANG) {
			langUrl = roxyMceAsset + '/lang/' + RoxyFilemanConf.LANG.substr(0, 2).toLowerCase() + '.json';
			if(!RoxyUtils.FileExists(langUrl)) {
				langUrl = '';
			}
		}
	...
	if(!langUrl) {
		langUrl = roxyMceAsset + '/lang/en.json';
	}
