...
function Directory(fullPath, numDirs, numFiles) {
	...
	this.GetHtml         = function() {
		...
		html += '<div><img src="' + roxyMceAsset + '/images/' + (this.dirs > 0 ? 'dir-plus.png' : 'blank.gif') + '" class="dirPlus" width="9" height="9">';
		html += '<img src="' + roxyMceAsset + '/images/folder.png" class="dir"><span class="name">' + this.name + ' (' + this.files + ')</span></div>';
		html += '</li>';
	...
	this.Select          = function(selectedFile) {
		...
		el.children('img.dir').prop('src', roxyMceAsset + '/images/folder.png');
		this.SetStatusBar();
	...
	this.SetOpened       = function() {
		...
		if(li.find('li').length < 1) {
			li.children('div').children('.dirPlus').prop('src', roxyMceAsset + '/images/blank.gif');
		} else if(this.IsExpanded()) {
			li.children('div').children('.dirPlus').prop('src', roxyMceAsset + '/images/dir-minus.png');
		} else {
			li.children('div').children('.dirPlus').prop('src', roxyMceAsset + '/images/dir-plus.png');
		}
	};
